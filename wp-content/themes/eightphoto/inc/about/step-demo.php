...
<div class="warning-msg"><?php echo __('Please install Recommended Plugins for demo to be imported completely.<br /> And Make sure you download demo in a fresh install. <br /> Your Old Data might be deleted.','eightphoto');?></div>
<?php
...
foreach($req_plugins as $slug=>$plugin) :
	if($plugin['bundled'] == false) {
		...
		<div class="action-tab warning">
			<h3><?php printf( esc_html__("Install : %s Plugin", 'eightphoto'), $plugin['name'] ); ?></h3>
			<p><?php echo __('Please check the plugins folder inside theme and upload the zip of plugins from plugin uploader.','eightphoto');?></p>
		</div>
		<?php
	} else {
		...
		switch($status['needs']) {
			...
			$label = esc_html__('Install and Activate', 'eightphoto');
			$link = $plugin['location'];
			break;

			case 'deactivate' :
			$btn_class = 'button';
			$label = esc_html__('Deactivate', 'eightphoto');
			$link = admin_url('plugins.php');
			break;

			case 'activate' :
			$btn_class = 'activate-offline button button-primary';
			$label = esc_html__('Activate', 'eightphoto');
			$link = $plugin['location'];
			break;
		}
		?>
